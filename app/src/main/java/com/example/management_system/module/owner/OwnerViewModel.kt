...
class OwnerViewModel : BaseViewModel<OwnerRepository>() {
    companion object {
    ...
    fun onRefresh() {
        ...
        getOwners(mCurrentPage, PAGE_SIZE)
    }
    ...
    private fun getOwners(pageNo: Int, pageSize: Int) {
        quickLaunch<OwnerList> {
            onStart { showLoading() }
            request {
                repository.getOwners(pageNo, pageSize)
            }
            onSuccess {
                // 旧数据
